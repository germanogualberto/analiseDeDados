...
```{r}
gastos4 = dadosCEAP %>% select(nomeParlamentar,valorLíquido,sgUF)%>%
...
```{r}
gastos5 = dadosCEAP %>% select(sgUF,valorLíquido,tipoDespesa)%>%
...
```{r}
gastos6 = dadosCEAP %>% select (tipoDespesa,sgPartido,valorLíquido)%>%
